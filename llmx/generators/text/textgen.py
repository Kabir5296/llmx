...
from .custom_served_textgen import CustomServedTextGen
import logging
...
def sanitize_provider(provider: str):
    if provider.lower() == "openai" or provider.lower() == "default" or provider.lower() == "azureopenai" or provider.lower() == "azureoai":
        ...
        return "anthropic"
    elif provider.lower() == "customserved":
        return "customserved"
    else:
        raise ValueError(
            f"Invalid provider '{provider}'. Supported providers are 'openai', 'hf', 'palm', 'cohere', 'anthropic' and 'customserved'."
        )
...
def llm(provider: str = None, **kwargs):
    ...
    if provider.lower() == "openai":
        ...
        return CustomServedTextGen(**kwargs)
